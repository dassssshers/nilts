...
You can configure all lint rules provided by `nilts_flutter_hooks` in the `analysis_options.yaml` file.
Choose one of the following configuration strategies.

### Disabling strategy

All `nilts_flutter_hooks` rules are enabled by default.
...
**NOTE: If you set `enable_all_lint_rules` to `false`, all lint rules that depend on `custom_lint` (not just nilts_flutter_hooks rules) will be disabled by default.**
